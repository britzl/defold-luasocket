...
static void LuaInit(lua_State* L) {
    ...
    luaL_register(L, MODULE_NAME, func);
    lua_pop(L, 1);
    assert(top == lua_gettop(L));
}

dmExtension::Result AppInitializeMimeExtension(dmExtension::AppParams* params) {
    return dmExtension::RESULT_OK;
}

dmExtension::Result InitializeMimeExtension(dmExtension::Params* params) {
	LuaInit(params->m_L);
...
dmExtension::Result AppFinalizeMimeExtension(dmExtension::AppParams* params) {
    return dmExtension::RESULT_OK;
}

dmExtension::Result FinalizeMimeExtension(dmExtension::Params* params) {
    return dmExtension::RESULT_OK;
}

DM_DECLARE_EXTENSION(Mime, LIB_NAME, AppInitializeMimeExtension, AppFinalizeMimeExtension, InitializeMimeExtension, 0, 0, FinalizeMimeExtension)
